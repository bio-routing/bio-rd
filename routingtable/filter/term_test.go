...
func TestProcess(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "empty from",
			prefix: net.NewPfx(net.IPv4FromOctets(100, 64, 0, 1).Ptr(), 8).Ptr(),
			path:   &route.Path{},
			from:   []*TermCondition{},
			then: []actions.Action{
		...
		},
		{
			...
			from: []*TermCondition{
				NewTermConditionWithPrefixLists(
					NewPrefixList(net.NewPfx(net.IPv4FromOctets(100, 64, 0, 1).Ptr(), 8).Ptr())),
			},
			then: []actions.Action{
				&actions.AcceptAction{},
			},
		...
		},
		{
			...
			from: []*TermCondition{
				NewTermConditionWithRouteFilters(
					NewRouteFilter(net.NewPfx(net.IPv4FromOctets(100, 64, 0, 1).Ptr(), 8).Ptr(), NewExactMatcher())),
			},
			then: []actions.Action{
				&mockAction{},
				&actions.AcceptAction{},
			...
			from: []*TermCondition{
				{
					prefixLists: []*PrefixList{
						NewPrefixListWithMatcher(NewExactMatcher(), net.NewPfx(net.IPv4(0).Ptr(), 32).Ptr()),
						NewPrefixList(net.NewPfx(net.IPv4FromOctets(100, 64, 0, 1).Ptr(), 8).Ptr()),
					},
				},
			},
