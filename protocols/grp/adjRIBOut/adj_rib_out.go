...
func (a *AdjRIBOut) AddPath(pfx *bnet.Prefix, p *route.Path) error {
	p, redist := p.CheckRedistribute(route.GRPPathType)
	if redist {
...
func (a *AdjRIBOut) redistributePath(p *route.Path) error {
	...
	p.GRPPath = &route.GRPPath{}

	var err error

	switch p.RedistributedFrom {
	case route.BGPPathType:
		err = a.redistributeFromBGP(p)
	case route.StaticPathType:
		err = a.redistributeFromStatic(p)
	default:
		return fmt.Errorf("redistribution from %s to GRP is not supported (yet?)", route.GetPathTypeName(p.Type))
	}

	if err != nil {
		return fmt.Errorf("error while redistributing path to GRP: %v", err)
	}

	return nil
}

func (a *AdjRIBOut) redistributeFromBGP(p *route.Path) error {
	if p.BGPPath == nil || p.BGPPath.BGPPathA == nil {
		return fmt.Errorf("BGP path or attributes nil")
	}

	p.GRPPath.NextHop = p.BGPPath.GetNextHop()

	return nil
}

func (a *AdjRIBOut) redistributeFromStatic(p *route.Path) error {
	if p.StaticPath == nil {
		return fmt.Errorf("static path nil")
	}

	p.GRPPath.NextHop = p.StaticPath.NextHop
