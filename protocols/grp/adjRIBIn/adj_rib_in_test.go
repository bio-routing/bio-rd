...
func TestAddPath(t *testing.T) {
	...
	for _, test := range tests {
		...
		adjRIBIn := New(filter.NewAcceptAllFilterChain(), vrf)
...
func TestRemovePath(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			routes: []*route.Route{
				...
				route.NewRoute(net.NewPfx(net.IPv4FromOctets(172, 16, 0, 0), 12).Ptr(), &route.Path{
					Type: route.GRPPathType,
					GRPPath: &route.GRPPath{
						NextHop: net.IPv4FromOctets(20, 0, 0, 0).Ptr(),
					},
			...
			expected: []*route.Route{
				route.NewRouteAddPath(net.NewPfx(net.IPv4FromOctets(172, 16, 0, 0), 12).Ptr(), []*route.Path{
					{
						Type: route.GRPPathType,
						GRPPath: &route.GRPPath{
							NextHop: net.IPv4FromOctets(20, 0, 0, 0).Ptr(),
	...
	for _, test := range tests {
		adjRIBIn := New(filter.NewAcceptAllFilterChain(), vrf.NewUntrackedVRF("vrf0", 0))
		for _, route := range test.routes {
			adjRIBIn.AddPath(route.Prefix().Ptr(), route.Paths()[0])
		}
